...
job_name = mol_name + '_' + density_type + '_' + filt_singles
job_string = """#!/bin/bash
#PBS -N %s
#PBS -l walltime=%s
#PBS -l %s
...
echo ""
cd $PBS_O_WORKDIR
python3 input_mvg.py %s %s %s &>> %s 
""" % (job_name, walltime, processors, queue, mol_name, density_type, filt_singles, output_file)
