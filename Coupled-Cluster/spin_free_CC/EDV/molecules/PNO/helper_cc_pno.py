...
class helper_localize(object):
    def __init__(self, wfn, F, MO, mol, pno_cutoff):
        ...
        self.wfn = wfn
        self.mol = mol
        self.pno_cutoff = pno_cutoff
        basis_ = self.wfn.basisset()
        self.nao = basis_.nbf()
        self.nmo = self.wfn.nmo()
        self.occ = self.wfn.doccpi()[0]
        self.vir = self.nmo - self.occ
        ...
        for ij in range(n_pairs):
            self.Tij.append(self.t2[ij])
            self.Ttij.append(self.t2_t[ij])
        ...
        # Print some stats
        print('Number of Pairs: %d'%n_pairs)
        avg_pno = 0
        for ij in range(n_pairs):
            # ignore weak pairs for now
            avg_pno += self.dim_[ij]
        avg_pno /= n_pairs    
