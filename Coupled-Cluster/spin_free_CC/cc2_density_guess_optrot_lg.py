...
def optrot_density(mol, rhf_e, rhf_wfn, corr_wfn, maxiter_cc, maxiter_lambda, maxiter_pert, inhom_pert, memory):
    ...
    if corr_wfn == 'CCSD':
        ...
        cc_wfn.compute_energy(1e-10, maxiter_cc)
    ...
    for p in range(0,1):
        ...
        string_Mu = "MU_" + cart[p]
        string_L = "L_" + cart[p]
        Muij[string_Mu] = Mu[string_Mu][cc_wfn.slice_o, cc_wfn.slice_o]
        Muab[string_Mu] = Mu[string_Mu][cc_wfn.slice_v, cc_wfn.slice_v]
        Muia[string_Mu] = Mu[string_Mu][cc_wfn.slice_o, cc_wfn.slice_v]
        Muai[string_Mu] = Mu[string_Mu][cc_wfn.slice_v, cc_wfn.slice_o]

        Lij[string_L] = L[string_L][cc_wfn.slice_o, cc_wfn.slice_o]
        Lab[string_L] = L[string_L][cc_wfn.slice_v, cc_wfn.slice_v]
        Lia[string_L] = L[string_L][cc_wfn.slice_o, cc_wfn.slice_v]
        Lai[string_L] = L[string_L][cc_wfn.slice_v, cc_wfn.slice_o]

   
        # Occupied - Occupied block of Density #
        
        Dij_Mu[string_Mu]  =  np.einsum('ia,ja->ij', ccpert[string_Mu].x1, cclambda.l1)
        Dij_Mu[string_Mu] +=  np.einsum('ia,ja->ij', cc_wfn.t1, ccpert[string_Mu].y1)
        Dij_Mu[string_Mu] +=  np.einsum('ikab,jkab->ij', cc_wfn.t2, ccpert[string_Mu].y2)
        ...
        Dab_Mu[string_Mu] +=  np.einsum('ia,ib->ba', cc_wfn.t1, ccpert[string_Mu].y1)
        Dab_Mu[string_Mu] +=  np.einsum('ijac,ijbc->ba', cc_wfn.t2, ccpert[string_Mu].y2)
        ...
        Dia_Mu[string_Mu] +=  2.0 * np.einsum('imae,me->ia', cc_wfn.t2, ccpert[string_Mu].y1)
        Dia_Mu[string_Mu] += -1.0 * np.einsum('imea,me->ia', cc_wfn.t2, ccpert[string_Mu].y1)
        ...
        Dia_Mu[string_Mu] += -1.0 * np.einsum('ie,ma,me->ia', cc_wfn.t1, cc_wfn.t1, ccpert[string_Mu].y1)
        Dia_Mu[string_Mu] += -1.0 * np.einsum('ie,ma,me->ia', cc_wfn.t1, ccpert[string_Mu].x1, cclambda.l1)
        Dia_Mu[string_Mu] += -1.0 * np.einsum('ie,ma,me->ia', ccpert[string_Mu].x1, cc_wfn.t1, cclambda.l1)
        
        # 4_th term
        
        Dia_Mu[string_Mu] += -1.0 * np.einsum('mnef,inef,ma->ia', cclambda.l2, cc_wfn.t2, ccpert[string_Mu].x1)
        Dia_Mu[string_Mu] += -1.0 * np.einsum('mnef,inef,ma->ia', cclambda.l2, ccpert[string_Mu].x2, cc_wfn.t1)
        Dia_Mu[string_Mu] += -1.0 * np.einsum('mnef,inef,ma->ia', ccpert[string_Mu].y2, cc_wfn.t2, cc_wfn.t1)
        
        # 5th term
        
        Dia_Mu[string_Mu] += -1.0 * np.einsum('mnef,mnaf,ie->ia', cclambda.l2, cc_wfn.t2, ccpert[string_Mu].x1)
        Dia_Mu[string_Mu] += -1.0 * np.einsum('mnef,mnaf,ie->ia', cclambda.l2, ccpert[string_Mu].x2, cc_wfn.t1)
        Dia_Mu[string_Mu] += -1.0 * np.einsum('mnef,mnaf,ie->ia', ccpert[string_Mu].y2, cc_wfn.t2, cc_wfn.t1)
        ...
        Dij_L[string_L] +=  np.einsum('ia,ja->ij', cc_wfn.t1, ccpert[string_L].y1)
        Dij_L[string_L] +=  np.einsum('ikab,jkab->ij', cc_wfn.t2, ccpert[string_L].y2)
        ...
        Dab_L[string_L] +=  np.einsum('ia,ib->ba', cc_wfn.t1, ccpert[string_L].y1)
        Dab_L[string_L] +=  np.einsum('ijac,ijbc->ba', cc_wfn.t2, ccpert[string_L].y2)
        ...
        Dia_L[string_L] +=  2.0 * np.einsum('imae,me->ia', cc_wfn.t2, ccpert[string_L].y1)
        Dia_L[string_L] += -1.0 * np.einsum('imea,me->ia', cc_wfn.t2, ccpert[string_L].y1)
        ...
        Dia_L[string_L] += -1.0 * np.einsum('ie,ma,me->ia', cc_wfn.t1, cc_wfn.t1, ccpert[string_L].y1)
        Dia_L[string_L] += -1.0 * np.einsum('ie,ma,me->ia', cc_wfn.t1, ccpert[string_L].x1, cclambda.l1)
        Dia_L[string_L] += -1.0 * np.einsum('ie,ma,me->ia', ccpert[string_L].x1, cc_wfn.t1, cclambda.l1)

        # 4th term

        Dia_L[string_L] += -1.0 * np.einsum('mnef,inef,ma->ia', cclambda.l2, cc_wfn.t2, ccpert[string_L].x1)
        Dia_L[string_L] += -1.0 * np.einsum('mnef,inef,ma->ia', cclambda.l2, ccpert[string_L].x2, cc_wfn.t1)
        Dia_L[string_L] += -1.0 * np.einsum('mnef,inef,ma->ia', ccpert[string_L].y2, cc_wfn.t2, cc_wfn.t1)

        # 5th term

        Dia_L[string_L] += -1.0 * np.einsum('mnef,mnaf,ie->ia', cclambda.l2, cc_wfn.t2, ccpert[string_L].x1)
        Dia_L[string_L] += -1.0 * np.einsum('mnef,mnaf,ie->ia', cclambda.l2, ccpert[string_L].x2, cc_wfn.t1)
        Dia_L[string_L] += -1.0 * np.einsum('mnef,mnaf,ie->ia', ccpert[string_L].y2, cc_wfn.t2, cc_wfn.t1)
        ...
        Dij[string_Mu+string_L] =  np.einsum('ia,ja->ij', ccpert[string_Mu].x1, ccpert[string_L].x1)
        Dij[string_Mu+string_L] += 2.0 * np.einsum('ikab,jkab->ij', ccpert[string_Mu].x2, ccpert[string_L].x2)
        Dij[string_Mu+string_L] -= np.einsum('ikab,kjab->ij', ccpert[string_Mu].x2, ccpert[string_L].x2)
        Dij[string_Mu+string_L] =  -1.0 * Dij[string_Mu + string_L]

        Dab[string_Mu+string_L] =   np.einsum('ia,ib->ab', ccpert[string_Mu].x1, ccpert[string_L].x1)
        Dab[string_Mu+string_L] +=  2.0 * np.einsum('ijac,ijbc->ab', ccpert[string_Mu].x2, ccpert[string_L].x2)
        Dab[string_Mu+string_L] -=  np.einsum('ijac,ijcb->ab', ccpert[string_Mu].x2, ccpert[string_L].x2)
        ...
        for p in range(0,1):
            ...
            for q in range(0,1):
                if p == q:
                    ...
                    print('\n optrot_response = %20.15lf %s \n' % (( 0.5 * optrot_PQ[str_pq] - 0.5 * optrot_PQ[str_qp]), corr_wfn))
...
polar1, Emat_ij, Emat_ab = optrot_density(mol, rhf_e, rhf_wfn, 'CC2', 1, 0, 2, 'true', 4)
...
for k in range(frz_vir, frz_vir+1):
    ...
    #optrot, tmp_ij, tmp_ab = optrot_density(mol, rhf_e, rhf_wfn, 'CCSD', 100, 100, 100, 'false', 4)
    optrot, tmp_ij, tmp_ab = optrot_density(mol, rhf_e, rhf_wfn, 'CC2', 100, 100, 100, 'true', 4)
