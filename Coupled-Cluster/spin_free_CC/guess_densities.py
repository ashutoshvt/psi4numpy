...
from helper_macro_cc import helper_ccenergy
from helper_macro_cc import helper_cchbar
from helper_macro_cc import helper_cclambda
from helper_macro_cc import helper_ccpert
from helper_macro_cc import helper_cclinresp

from helper_cc import helper_ccenergy as helper_ccenergy1 
from helper_cc import helper_cchbar as helper_cchbar1
from helper_cc import helper_cclambda as helper_cclambda1
from helper_cc import helper_ccpert as helper_ccpert1
from helper_cc import helper_cclinresp as helper_cclinresp1
...
def optrot_density(mol, rhf_e, rhf_wfn, maxiter_cc, maxiter_lambda, maxiter_pert, memory, density):
    ...
    dipole_array = mints.so_dipole()
    angmom_array = mints.so_angular_momentum()
    C = rhf_wfn.Ca()
    npC = np.asarray(C)
    nbf = rhf_wfn.nmo()
    ...
    for p in range(0,1):
        ...
        string_L = "L_" + cart[p]
        Mu[string_Mu] = np.einsum('uj,vi,uv', npC, npC, np.asarray(dipole_array[p]))
        L[string_L] = -0.5 * np.einsum('uj,vi,uv', npC, npC, np.asarray(angmom_array[p]))

        Muij[string_Mu] = Mu[string_Mu][:ndocc, :ndocc]
        Muab[string_Mu] = Mu[string_Mu][ndocc:, ndocc:]
        Muia[string_Mu] = Mu[string_Mu][:ndocc, ndocc:]
        Muai[string_Mu] = Mu[string_Mu][ndocc:, :ndocc]

        Lij[string_L] = L[string_L][:ndocc, :ndocc]
        Lab[string_L] = L[string_L][ndocc:, ndocc:]
        Lia[string_L] = L[string_L][:ndocc, ndocc:]
        Lai[string_L] = L[string_L][ndocc:, :ndocc]


    if density == 'TDHF':

        Muia_copy={}; Lia_copy={}; x1={}
        ...
        for p in range(0,1):
            ...
            Muia_copy[string_Mu] = Muia[string_Mu].copy()
            Lia_copy[string_L] = Lia[string_L].copy()

            Muia_copy[string_Mu] *= -2
            Lia_copy[string_L] *= -2

            Muia_copy[string_Mu]  = Muia_copy[string_Mu].ravel() 
            Lia_copy[string_L]    = Lia_copy[string_L].ravel()

            x1[string_Mu]  = np.linalg.solve(A11 - B11, Muia_copy[string_Mu])
            x1[string_L]   = np.linalg.solve(A11 - B11, Lia_copy[string_L])

            print(x1[string_Mu])
            print(x1[string_L])
            return

            #x1[string_Mu] =  x1[string_Mu].ravel()
            #x1[string_L] =  x1[string_L].ravel()

            #static_elec_polar[cart[p]] = 2 * np.einsum('p,p->', Muia_copy[string_Mu], x1[string_Mu]) 
            #static_mag_polar[cart[p]] = 2 * np.einsum('p,p->', Lia_copy[string_L], x1[string_L]) 

    elif density == 'CIS_D':

        Muia_copy={}; Lia_copy={}; x1={}; x2={}
        Co = rhf_wfn.Ca_subset("AO", "OCC")
        Cv = rhf_wfn.Ca_subset("AO", "VIR")
        epsilon = np.asarray(rhf_wfn.epsilon_a())
        nov = ndocc * nvir
        ...
        v_ijab = np.asarray(mints.mo_transform(I, Co, Co, Cv, Cv))
        v_iajb = np.asarray(mints.mo_transform(I, Co, Cv, Co, Cv))
        v_acbj = np.asarray(mints.mo_transform(I, Cv, Cv, Cv, Co))
        v_kjai = np.asarray(mints.mo_transform(I, Co, Co, Cv, Co))
        ...
        for p in range(0,1):
            ...
            Muia_copy[string_Mu]  = Muia_copy[string_Mu].ravel()
            Lia_copy[string_L]    = Lia_copy[string_L].ravel()
            x1[string_Mu] = np.linalg.solve(A11, Muia_copy[string_Mu])
            x1[string_L]  = np.linalg.solve(A11, Lia_copy[string_L])

            print(x1[string_Mu])
            print(x1[string_L])

            x1[string_Mu].shape = (ndocc,nvir)
            x1[string_L].shape = (ndocc,nvir)

            x2[string_Mu] =  np.einsum('acbj,ic->ijab', v_acbj, x1[string_Mu])
            x2[string_Mu] += np.einsum('acbi,jc->ijab', v_acbj, x1[string_Mu])
            x2[string_Mu] -= np.einsum('kjai,kb->ijab', v_kjai, x1[string_Mu])
            x2[string_Mu] -= np.einsum('kjbi,ka->ijab', v_kjai, x1[string_Mu])

            x2[string_L] =  np.einsum('acbj,ic->ijab', v_acbj, x1[string_L])
            x2[string_L] += np.einsum('acbi,jc->ijab', v_acbj, x1[string_L])
            x2[string_L] -= np.einsum('kjai,kb->ijab', v_kjai, x1[string_L])
            x2[string_L] -= np.einsum('kjbi,ka->ijab', v_kjai, x1[string_L])

            Dij[string_Mu+string_L] =  np.einsum('ia,ja->ij', x1[string_Mu], x1[string_L])
            Dij[string_Mu+string_L] += 2.0 * np.einsum('ikab,jkab->ij', x2[string_Mu], x2[string_L])
            Dij[string_Mu+string_L] -= np.einsum('ikab,kjab->ij', x2[string_Mu], x2[string_L])
            Dij[string_Mu+string_L] =  -1.0 * Dij[string_Mu + string_L]

            Dab[string_Mu+string_L] =   np.einsum('ia,ib->ab', x1[string_Mu], x1[string_L])
            Dab[string_Mu+string_L] +=  2.0 * np.einsum('ijac,ijbc->ab', x2[string_Mu], x2[string_L])
            Dab[string_Mu+string_L] -=  np.einsum('ijac,ijcb->ab', x2[string_Mu], x2[string_L])
    ...
    elif density == 'macro_cc':
        ccsd = helper_ccenergy(mol, rhf_e, rhf_wfn, memory)
    else :
        ccsd = helper_ccenergy1(mol, rhf_e, rhf_wfn, memory)
        ...
        if density == 'macro_cc':
            ...
            cchbar = helper_cchbar1(ccsd)
            cclambda = helper_cclambda1(ccsd,cchbar)
        ...
        for p in range(0,1):
             ...
             if density == 'macro_cc':
                 ...
                 ccpert[string_Mu] = helper_ccpert1(string_Mu, Mu[string_Mu], ccsd, cchbar, cclambda, omega)
                 ccpert[string_L] = helper_ccpert1(string_L, L[string_L], ccsd, cchbar, cclambda, omega)
...
#polar1, Emat_ij, Emat_ab   = optrot_density(mol, rhf_e, rhf_wfn, 100, 100, 100, 4, 'full')
#polar1, Emat_ij, Emat_ab = optrot_density(mol, rhf_e, rhf_wfn, 1, 0, 1, 4, 'TDHF')
#polar1, Emat_ij, Emat_ab = optrot_density(mol, rhf_e, rhf_wfn, 1, 0, 1, 4, 'guess')
polar1, Emat_ij, Emat_ab = optrot_density(mol, rhf_e, rhf_wfn, 0, 0, 1, 4, 'full')
#zer_optrot, Emat_ij, Emat_ab = optrot_density(mol, rhf_e, rhf_wfn, 0, 0, 0, 4, 'CIS_D')
#optrot_density(mol, rhf_e, rhf_wfn, 1, 0, 1, 4, 'TDHF')
"""frz_vir = 15
...
for k in range(frz_vir, frz_vir+1):
    ...
    optrot, tmp_ij, tmp_ab = optrot_density(mol, rhf_e, rhf_wfn, 100, 100, 100, 4, 'full')
